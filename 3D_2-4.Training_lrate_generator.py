...
from Modules.Prototypes.SlidingWindowBatchGenerator import *
...
def train_and_predict(use_existing):
  ...
  originFile_list = sorted(glob(cm.workingPath.training3DSet_path + 'img*.npy'))[:]
  maskFile_list = sorted(glob(cm.workingPath.training3DSet_path + 'mask*.npy'))[:]

  # valoriginFile_list = sorted(glob(cm.workingPath.validationSet_path + 'img*.npy'))[:]
  # valmaskFile_list = sorted(glob(cm.workingPath.validationSet_path + 'mask*.npy'))[:]
  ...
  model_best_checkpoint = callbacks.ModelCheckpoint(bestfilepath, monitor='val_loss', verbose=0, save_best_only=False)
  ...
  for i in range(len(originFile_list)):
    temp = np.load(originFile_list[i])

    xTrain.append(temp)

    temp = np.load(maskFile_list[i])

    yTrain.append(temp)

  # for i in range(len(valoriginFile_list)):
  #     temp = np.load(valoriginFile_list[i])
  #
  #     xValid.append(temp)
  #
  #     temp = np.load(valmaskFile_list[i])
  #
  #     yValid.append(temp)
