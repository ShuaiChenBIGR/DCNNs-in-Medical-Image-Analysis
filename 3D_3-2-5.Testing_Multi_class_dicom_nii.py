...
def model_test(use_existing):
  ...
  for i in range(len(originFile_list)):
    ...
    nib.save(nii_temp, current_dir + '/NIFTY/masksAortaGroundTruth.nii.gz')

    nii_temp = nib.Nifti1Image(np.swapaxes(np.uint16(final_test_masks[:, ::-1, ::-1, 2]), 0, 2), nii_space)
    nib.save(nii_temp, current_dir + '/NIFTY/masksPulGroundTruth.nii.gz')
    ...
    nii_temp = nib.Nifti1Image(np.swapaxes(np.uint16(final_test_aorta_predicted_threshold[:, ::-1, ::-1]), 0, 2), nii_space)
    nib.save(nii_temp, current_dir + '/NIFTY/masksAortaPredicted_threshold.nii.gz')

    nii_temp = nib.Nifti1Image(np.swapaxes(np.uint16(final_test_pul_predicted_threshold[:, ::-1, ::-1]), 0, 2), nii_space)
    nib.save(nii_temp, current_dir + '/NIFTY/masksPulPredicted_threshold.nii.gz')
    ...
    nii_temp = nib.Nifti1Image(np.swapaxes(np.uint16(final_test_images[:, ::-1, ::-1]) + 4000, 0, 2), nii_space)
    nib.save(nii_temp, current_dir + '/NIFTY/OriginalImages.nii.gz')
    ...
    final_test_aorta_predicted = np.ndarray([sum(vol_slices), 512, 512], dtype=np.float32)
    final_test_pul_predicted = np.ndarray([sum(vol_slices), 512, 512], dtype=np.float32)
    ...
    nii_temp = nib.Nifti1Image(np.swapaxes(final_test_aorta_predicted[:, ::-1, ::-1], 0, 2), nii_space)
    nib.save(nii_temp, current_dir + '/NIFTY/masksAortaPredicted.nii.gz')

    nii_temp = nib.Nifti1Image(np.swapaxes(final_test_pul_predicted[:, ::-1, ::-1], 0, 2), nii_space)
    nib.save(nii_temp, current_dir + '/NIFTY/masksPulPredicted.nii.gz')
