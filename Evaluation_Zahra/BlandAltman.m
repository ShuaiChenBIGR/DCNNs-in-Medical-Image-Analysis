...
%  {default = {'RPC(%)';'CV'} }
%
% 'limits' - specifies the axes limits:
...
% set(txt(1:end),'VerticalAlignment', 'Middle','FontSize',15);
...
% default values
params.corrInfo = {'ICC';'r';'r2'}; %default {'eq';'r2';'SSE';'n'};
params.baInfo = {};  %{'RPC(%)'; 'ICC';'r';'r2'};  %default  {'RPC(%)';'CV'};
...
params.baStatsMode = 'Normal';  % default 'Normal';
...
while length(varargin)>i
	...
	switch upper(parameter)
		...
		case 'BAYLIMODE', params.baYLimMode = val;
		case 'LEVEL' , params.Level = {val}; %%added by Zahra
%         case 'ICc', params.ICC= val;
%         case 'r', params.r = val;
%         case 'r2', params.r2 = val;
%             
...
if iscell(label)
	...
	elseif length(label)==2
		...
		params.deltaLabel = [label{2} ' - ' label{1} ' (' diffUnits ')'];
	end	
else
	params.d1Label = label;
	params.d2Label = label;
	params.meanLabel = label;
	params.deltaLabel = ['\Delta ' label];
end
...
if isempty(fig)
	fig = figure;
	set(fig,'units','centimeters','position',[3 3 40 20],'color','w');  % position by zahra
	cAH = subplot(121);
    set(gca,'fontweight','bold', 'FontSize',18,'linew',2)  % added by Zahra to increase the txtx size

	baAH = subplot(122);
    size(baAH)
    set(gca,'fontweight','bold', 'FontSize',18,'linew',2) % added by Zahra to increase the txt size
...
r = corrcoef(data.maskedSet1,data.maskedSet2);
% added by zahra to have the ICC on the plot as well
[Icc, LB, UB, F, df1, df2, pvalue] = ICC_v2([data.maskedSet1,data.maskedSet2], 'A-1' ); %added by zahra
stats.r=r(1,2);
stats.r2 = stats.r^2;
stats.ICC=Icc; %added by zahra
...
for i=1:length(params.corrInfo)
	switch upper(params.corrInfo{i})
        ...
        case 'ICC', corrtext = [corrtext; ['ICC=' mynum2str(stats.ICC,4)]];
...
text(params.axesLimits(1)+0.01*(params.axesLimits(2)-params.axesLimits(1)),params.axesLimits(1)+0.9*(params.axesLimits(2)-params.axesLimits(1)),corrtext,'parent',cAH,'FontSize',15,'fontweight','bold');
...
% stats.Level=params.Level;  %%Zahra
...
[~, stats.differenceMeanP] = ttest(data.maskedDifferences,0); %Pvalue of the differences
...
% added by zahra to have the ICC ,r,r^2 on the plot as well
r = corrcoef(data.maskedSet1,data.maskedSet2);
[Icc, LB, UB, F, df1, df2, pvalue] = ICC_v2([data.maskedSet1,data.maskedSet2], 'A-1' ); %added by zahra
stats.r=r(1,2);
stats.r2 = stats.r^2;
stats.ICC=Icc; % till here added by zahra
...
setat.skewness = skewness(data.maskedDifferences,1);
...
% changed by Zahra- commented this part to have the data in their own size
% not bigger
...
switch upper(params.baStatsMode)
	case {'NORMAL','GAUSSIAN'}
		...
		plot(a(1:2),stats.differenceMean-stats.rpc*[1 1],':k')
		text(a(2),stats.differenceMean+stats.rpc, [mynum2str(stats.differenceMean+stats.rpc,2) ' (+1.96SD)'],'HorizontalAlignment','left','VerticalAlignment','middle','fontsize',fontsize,'fontweight','bold');
		text(a(2),stats.differenceMean,[mynum2str(stats.differenceMean,2) ' [p=' mynum2str(stats.differenceMeanP,2) ']'],'HorizontalAlignment','left','VerticalAlignment','middle','fontsize',fontsize,'fontweight','bold');
		text(a(2),stats.differenceMean-stats.rpc, [mynum2str(stats.differenceMean-stats.rpc,2) ' (-1.96SD)'],'HorizontalAlignment','left','VerticalAlignment','middle','fontsize',fontsize,'fontweight','bold');
% 		text(a(2),a(4),['{\bfLevel: ' params.Level '}'],'interpreter','tex','HorizontalAlignment','right','VerticalAlignment','top','Parent',baAH);
	...
	case 'NON-PARAMETRIC'
		...
		plot(a(1:2),stats.differenceMedian-stats.rpcNP*[1 1],':k')
		text(a(2),stats.differenceMedian+stats.rpcNP, [mynum2str(stats.differenceMedian+stats.rpcNP,2) ' (+1.45IQR)'],'HorizontalAlignment','left','VerticalAlignment','middle','fontsize',fontsize,'fontweight','bold');
		text(a(2),stats.differenceMedian,[mynum2str(stats.differenceMedian,2) ' [p=' mynum2str(stats.differenceMedianP,2) ']'],'HorizontalAlignment','left','VerticalAlignment','middle','fontsize',fontsize,'fontweight','bold');
		text(a(2),stats.differenceMedian-stats.rpcNP, [mynum2str(stats.differenceMedian-stats.rpcNP,2) ' (-1.45IQR)'],'HorizontalAlignment','left','VerticalAlignment','middle','fontsize',fontsize,'fontweight','bold');
		if isGaussian(stats) 
...
BAtext = {};BAtext2 ={};
BAtext = [BAtext; [strjoin({'{\bf Level:', params.Level{1,1},'}'})]]; %%Zahra added
% BAtext = [strjoin({'{\bf Level:', params.Level{1,1},'}'})]; %%Zahra added
...
for i=1:length(params.baInfo)
	switch upper(params.baInfo{i})
        ...
        case 'ICC', BAtext2 = [BAtext2; ['ICC=' mynum2str(stats.ICC,4)]];
        case 'R', BAtext2 = [BAtext2; ['r=' mynum2str(stats.r,4)]];
        case 'R2', BAtext2 = [BAtext2; ['r^2=' mynum2str(stats.r^2,4)]];
		
    end
end
a = axis(baAH);
% commented by Zahra
text(a(2),a(4),BAtext,'interpreter','tex','HorizontalAlignment','right','VerticalAlignment','top','Parent',baAH,'FontSize',15,'fontweight','bold');
text(a(1)+0.5,a(4),BAtext2,'interpreter','tex','HorizontalAlignment','left','VerticalAlignment','top','Parent',baAH,'FontSize',15,'fontweight','bold');
% text(a(1)+0.5,a(4),BAtext,'interpreter','tex','HorizontalAlignment','left','VerticalAlignment','top','Parent',baAH,'FontSize',15,'fontweight','bold');
