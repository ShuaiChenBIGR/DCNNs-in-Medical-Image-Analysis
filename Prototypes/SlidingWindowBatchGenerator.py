...
from Modules.CommonUtil.Constants import *
from Modules.Prototypes.SlidingWindowImages import *
...
class SlidingWindowBatchGenerator(image.Iterator):
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        Xdata_batch = np.ndarray([num_images_batch] + list(self.size_image) + [1], dtype=FORMATIMAGEDATA)
        Ydata_batch = np.ndarray([num_images_batch] + list(self.size_image) + [3], dtype=FORMATMASKDATA)
        ...
        for i, index in enumerate(index_array):
            ...
            Xdata_batch[i] = self.list_Xdata[idx_fullimage][z_back:z_front, x_left:x_right, y_down:y_up].reshape(list(self.size_image) + [1])
    ...
    def getArrayDims(self, num_batches, num_channels=1):
        if K.image_data_format() == 'channels_first':
            return [num_batches, num_channels] + list(self.size_image)
        else:
            return [num_batches] + list(self.size_image) + [num_channels]
